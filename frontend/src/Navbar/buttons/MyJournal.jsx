...
export default function MyJournal() {
  ...
  useEffect(() => {
    ...
    // if something is in storage converts it back to JS and stores it
    if (saved) {
  ...
  const handleSave = () => {
    ...
    // keeps old id if editing OR makes new one if adding
    const newEntry = { ...form, id: editingId || Date.now() };

    // updates or adds the entry in the entries list
    setEntries((prev) =>
      editingId
  ...
  const handleDelete = (id) => {
    ...
    // if user was editing this entry, reset the form
    ...
    <div className="page-wrapper">
      <div className="page-container">
        ...
        <div className="section-block">
          {/* shows error only when a message exists */}
          ...
          <input
            ...
            value={form.symptom}
            onChange={updateField}
            className={form.symptom ? "text-input has-value" : "text-input"}
          />
        ...
        <div className="section-block">
          ...
          {/* if there are no entries yet, show empty state */}
          {entries.length === 0 ? (
            ...
            entries.map((entry) => (
              <div key={entry.id} className="journal-item">
                ...
                {entry.symptom || "(No symptom)"} ({entry.severity})
